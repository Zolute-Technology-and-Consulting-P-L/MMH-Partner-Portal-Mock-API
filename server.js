...
app.post(
  "/partner/orderv2/products/:orderID",
  ...
  function (req, res) {
    ...
    let partnerCommi = new Commission({
      commission_amount: $orderTotal / 10,
      ...
      matureDate: dateTime.dateTime,
      commissionStatus: "Matured",
    ...
    DraftOrder.updateOne(
      { _id: req.params.orderID },
      {
        $push: { products: { $each: productsArr } },
        pricing: $setPricing,
        orderCode: $orderCode,
      },
      { upsert: true },
...
app.post(
  "/partner/orderv2/priority/:orderID",
  auth.authenticateToken,
  function (req, res) {
    DraftOrder.findOne({ _id: req.params.orderID }).then((order) => {
      res.json(order);
...
app.post("/partner/orderv2/activationAmount/", function (req, res) {
  fs.readFile(
    __dirname + "/" + "json/activationAmount.json",
    "utf8",
    function (err, data) {
      res.send(data);
    }
...
app.post("/partner/orderv2/create/:orderID", function (req, res) {
  fs.readFile(
    __dirname + "/" + "json/createorder.json",
    "utf8",
    function (err, data) {
      res.send(data);
    }
...
app.post(
  "/partner/orderv2/activationAmount/:orderID",
  auth.authenticateToken,
  function (req, res) {
    DraftOrder.findOne({ _id: req.params.orderID }).then((order) => {
      res.json(order);
...
app.get("/partner/orderv2/:orderCode", function (req, res) {
  fs.readFile(
    __dirname + "/" + "json/orderdetails.json",
    "utf8",
    function (err, data) {
      res.send(data);
    }
...
app.get("/public/productsv2/:orderid", function (req, res) {
  fs.readFile(
    __dirname + "/" + "json/productsv2.json",
    "utf8",
    function (err, data) {
      res.send(data);
    }
...
app.get(
  "/partner/Commission/withdrawalRequest/",
  auth.authenticateToken,
  function (req, res) {
    let response = {
      totalRecords: 1,
...
app.post(
  "/partner/customer/verifyotp",
  auth.authenticateToken,
  function (req, res) {
    const { mobile, otp } = req.body;

    CustomerController.verifyotp({ mobile, otp })
      .then((customer) => {
        if (customer) {
